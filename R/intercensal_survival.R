...
intercensal_surv_feeney <- function(c1,
  ...
  # apply method
  if(interc_t == 5){
    surv_data$lx_l2.5 <- DemoTools::surv5(surv_data$c1, surv_data$c2)
  }
  if(interc_t == 10){
    surv_data$lx_l2.5 <- DemoTools::surv10(surv_data$c1, surv_data$c2)
  }
  if(!interc_t %in% c(5,10)){
    surv_data$lx_l2.5 <- DemoTools::survN(surv_data$c1, surv_data$c2, interval = interc_t)
  }
